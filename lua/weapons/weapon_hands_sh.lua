...
function SWEP:AddBlockPush(normal)
	...
	self.BlockPushVel = self.BlockPushVel + localDir * 40
			...
			if ply:KeyDown(IN_ATTACK) and !ply.organism.superfighter and !(org and ply.PlayerClassName == "furry" and org.owner.PlayerClassName != "furry") and !ply:IsBerserk() then
				...
				if org and bone == "ValveBiped.Bip01_Spine2" and trace.Hit then
					...
					if (self.CPRThink or 0) < CurTime() then
						...
						if org.alive then
							//org.o2[1] = math.min(org.o2[1] + hg.organism.OxygenateBlood(org) * 2 * (ply.Profession == "medic" and 2 or 1), org.o2.range)
							org.pulse = math.min(org.pulse + 5 * (ply.Profession == "medic" and 2 or 1),70)
							org.CO = math.Approach(org.CO, 0, (ply.Profession == "medic" and 2 or 1))
							org.COregen = math.Approach(org.COregen, 0, (ply.Profession == "medic" and 2 or 1))

							if math.random(3) == 1 then
								org.lungsfunction = true
							end

							if math.random(50) == 1 and (ply.Profession != "medic") then
...
function SWEP:PrimaryAttack(forcespecial)
	...
	if not IsFirstTimePredicted() then
		self:DoBFSAnimation(side, useClawHandsVisual(owner) and 1 or self.JabAnimTime)
		return
	end
	...
	self:SetNextPrimaryFire(CurTime() + self.SwingCooldown * math.Clamp((180 - owner.organism.stamina[1]) / 90,1,2) + (math.max(special_attack and 0.5 or 0, clawDelay)))
	self:SetNextSecondaryFire(CurTime() + self.SwingCooldown + (math.max(special_attack and 0.5 or 0, clawDelay)))
...
function SWEP:Reload()
	...
	if not isZombieHands(owner) then
		self:SetFists(false)
		self:SetBlocking(false)
	end
